...
bool preprocess(const QString& sourceFile, const QString& targetFile, const QStringList& includePaths,
  const QStringList& frameworkPaths)
{
  ...
  std::unique_ptr<simplecpp::TokenList> rawtokens(
    new simplecpp::TokenList(ba.constData(), ba.size(), files, {}, &outputList));
...
int main(int argc, char* argv[])
{
  ...
  if (args.contains("ast-to-xml")) {
    printf("Running ast-to-xml on file [%s] using pp_file [%s] and include-paths [%s]\n", qPrintable(fileName),
      qPrintable(pp_file), qPrintable(args.value("include-paths")));
  ...
  printf("Done, %d warnings (%d known issues)\n", ReportHandler::warningCount(), ReportHandler::suppressedCount());
}
