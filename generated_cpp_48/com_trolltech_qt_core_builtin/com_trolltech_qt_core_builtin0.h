...
void delete_QPoint(QPoint* obj) { delete obj; } 
   ...
   const QPoint  __mul__(QPoint* theWrappedObject, double  c);
   const QPoint  __mul__(QPoint* theWrappedObject, float  c);
   const QPoint  __mul__(QPoint* theWrappedObject, int  c);
   QPoint*  __imul__(QPoint* theWrappedObject, double  c);
   QPoint*  __imul__(QPoint* theWrappedObject, float  c);
   QPoint*  __imul__(QPoint* theWrappedObject, int  c);
...
{ Q_OBJECT
public:
Q_ENUMS(FormattingOption ParsingMode )
...
void delete_QUrl(QUrl* obj) { delete obj; } 
   ...
   QString  topLevelDomain(QUrl* theWrappedObject) const;
...
{ Q_OBJECT
public:
Q_ENUMS(AlignmentFlag AnchorAttribute AnchorPoint ApplicationAttribute ArrowType AspectRatioMode Axis BGMode BrushStyle CaseSensitivity CheckState ClipOperation ConnectionType ContextMenuPolicy CoordinateSystem Corner CursorMoveStyle CursorShape DateFormat DayOfWeek DockWidgetArea DockWidgetAreaSizes DropAction EventPriority FillRule FocusPolicy FocusReason GestureFlag GestureState GestureType GlobalColor HitTestAccuracy ImageConversionFlag InputMethodHint InputMethodQuery ItemDataRole ItemFlag ItemSelectionMode Key KeyboardModifier LayoutDirection MaskMode MatchFlag MouseButton NavigationMode Orientation PenCapStyle PenJoinStyle PenStyle ScrollBarPolicy ShortcutContext SizeHint SizeMode SortOrder TextElideMode TextFlag TextFormat TextInteractionFlag TileRule TimeSpec ToolBarArea ToolBarAreaSizes ToolButtonStyle TouchPointState TransformationMode UIEffect WhiteSpaceMode WidgetAttribute WindowFrameSection WindowModality WindowState WindowType )
Q_FLAGS(Alignment DockWidgetAreas DropActions GestureFlags ImageConversionFlags InputMethodHints InputMethodQueries ItemFlags KeyboardModifiers MatchFlags MouseButtons Orientations TextInteractionFlags ToolBarAreas TouchPointStates WindowStates WindowFlags )
enum AlignmentFlag{
...
enum MouseButton{
  NoButton = Qt::NoButton,   LeftButton = Qt::LeftButton,   RightButton = Qt::RightButton,   MidButton = Qt::MidButton,   MiddleButton = Qt::MiddleButton,   XButton1 = Qt::XButton1,   XButton2 = Qt::XButton2,   MouseButtonMask = Qt::MouseButtonMask};
enum NavigationMode{
