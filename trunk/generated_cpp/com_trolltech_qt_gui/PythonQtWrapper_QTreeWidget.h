...
{ public:
inline QStringList  mimeTypes() const { return QTreeWidget::mimeTypes(); }
inline bool  event(QEvent*  e) { return QTreeWidget::event(e); }
inline void dropEvent(QDropEvent*  event) { QTreeWidget::dropEvent(event); }
inline bool  dropMimeData(QTreeWidgetItem*  parent, int  index, const QMimeData*  data, Qt::DropAction  action) { return QTreeWidget::dropMimeData(parent, index, data, action); }
inline Qt::DropActions  supportedDropActions() const { return QTreeWidget::supportedDropActions(); }
};
